...
const EditProject = () => {
    ...
    const addFieldToForm = (formIndex) => {
        const updatedForms = [...forms];
        updatedForms[formIndex].fields.push({ fieldId: '', value: '' });
        setForms(updatedForms);
    };
    ...
    const handleFormSubmit = async (formIndex, projectId) => {
        try {
            ...
            if (form._id) {
                await doPUT(ENDPOINTS.updateForm(form._id), { ...form, project: projectId });
            } else {
                await doPOST(ENDPOINTS.addForm, { ...form, project: projectId });
            }
        } catch (error) {
    ...
    return (
        <div className='container'>
            ...
            {project?._id && (
                <div>
                    <button type="button" className="btn btn-secondary mb-2" onClick={addForm}>Add Form</button>
                    {forms?.map((form, formIndex) => (
                        <div key={formIndex} className="mb-3">
                            <form onSubmit={(e) => { e.preventDefault(); handleFormSubmit(formIndex, project._id); }}>
                                ...
                                />
                                <button type="button" className="btn btn-secondary mb-2" onClick={() => addFieldToForm(formIndex)}>Add Field</button>
                                {form.fields.map((field, fieldIndex) => (
                                    <div key={fieldIndex} className="input-group mb-2">
                                        <select
                                            className="form-select"
                                            value={field.fieldId}
                                            onChange={(e) => handleFieldChange(formIndex, fieldIndex, 'fieldId', e.target.value)}
                                        >
                                        ...
                                        <input
                                            ...
                                            className="form-control"
                                            value={field.value}
                                            onChange={(e) => handleFieldChange(formIndex, fieldIndex, 'value', e.target.value)}
